...
def _element_to_dict(data, position, as_class, tz_aware, uuid_subtype):
    ...
    value, position = _element_getter[element_type](data, position, as_class, tz_aware, uuid_subtype)
...
def _elements_to_dict(data, as_class, tz_aware, uuid_subtype):
    ...
    while position < end:
        (key, value, position) = _element_to_dict(data, position, as_class, tz_aware, uuid_subtype)
        # update by Nerve
        """
        if type(value) in [dict, list]:
            result[key] = value
        elif result.has_key(value):
            print 'repeat key'
...
def _bson_to_dict(data, as_class, tz_aware, uuid_subtype):
    ...
    return (_elements_to_dict(elements, as_class, tz_aware, uuid_subtype), data[obj_size:])
...
class BSON(binary_type):
    ...
    def decode(self, as_class=dict, tz_aware=False, uuid_subtype=OLD_UUID_SUBTYPE):
