    ...
    if restarts[4] == '*':
        ...
        if restarts[1] == "*" and  restarts[0] != "*":
            ...
            if isinstance(mmm,str):
                s = st + "/sc hourly /tn %s_%s " % (name,i)
                i += 1
                whs.append(s)
            else:
                for m in mmm:
                    s = st + "/sc hourly /tn %s_%s /st 00:%s" % (name,i,m)
                    i += 1
                    whs.append(s)
            ...
            if isinstance(hhh,str):
                s = st + "/sc minute /tn %s_%s /st %s:" % (name,i,hhh)
                i += 1
                whs.append(s)
            else:
                for h in hhh:
                    s = st + "/sc minute /tn %s_%s /st %s:00" % (name,i,h)
                    i += 1
                    whs.append(s)
        elif restarts[1] == "*" and  restarts[0] == "*":
            ...
            if isinstance(hhh,list) and isinstance(mmm,list):
                for x in hhh:
                    for m in mmm:
                        s = st + "/sc daily /tn %s_%s /st %s:%s" % (name,i,x,m)
                        i += 1
                        print '1'
                        whs.append(s)
            elif isinstance(hhh,list) and isinstance(mmm,str):
                for h in hhh:
                    s = st + "/sc daily /tn %s_%s /st %s:%s" % (name,i,h,mmm)
                    i += 1
                    print '12'
                    whs.append(s)
            elif isinstance(mmm,list) :
                for m in mmm:
                    s = st + "/sc daily /tn %s_%s /st %s:%s" % (name,i,hhh,m)
                    i += 1
                    print '13'
                    whs.append(s)
            else :
                s = st + "/sc daily /tn %s_%s /st %s:%s" % (name,i,hhh,mmm)
                print '14'
                whs.append(s)
        ...
        if restarts[1] == "*" and restarts[0] != "*":
            if restarts[0].find('*') == 0:
                ...
                mmm = minutes(restarts[0],minuteset)
            if isinstance(mmm,str):
                s = st + " /tn %s_%s /st 00:%s" % (name,i,mmm)
                i += 1
            else:
                for m in mmm:
                    s = st + " /tn %s_%s /st %s:%s" % (name,i,m)
                    i += 1
        elif restarts[1] != "*" and restarts[0] == "*":
            if restarts[1].find("*") == 0:
                hhh = restarts[1].split('/')[-1]
                ...
                hhh = hour(restarts[0],hourset)
            if isinstance(hhh,str):
                s = st + "/tn %s_%s /st %s:" % (name,i,hhh)
                i += 1
                whs.append(s)
            else:
                for h in hhh:
                    s = st + "/tn %s_%s /st %s:00" % (name,i,h)
                    i += 1
                    whs.append(s)
            ...
            if isinstance(hhh,list) and isinstance(mmm,list):
                for x in hhh:
                    for m in mmm:
                        s = st + "/tn %s_%s /st %s:%s" % (name,i,x,m)
                        i += 1
                        print '1'
                        whs.append(s)
            elif isinstance(hhh,list) and isinstance(mmm,str):
                for h in hhh:
                    s = st + "/tn %s_%s /st %s:%s" % (name,i,h,mmm)
                    i += 1
                    print '12'
                    whs.append(s)
            elif isinstance(mmm,list) :
                for m in mmm:
                    s = st + "/tn %s_%s /st %s:%s" % (name,i,hhh,m)
                    i += 1
                    print '13'
                    whs.append(s)
            else :
                s = st + "/tn %s_%s /st %s:%s" % (name,i,hhh,mmm)
                print '14'
                whs.append(s)
